...
fs_inode_truncate(struct fs_ctx *ctx, uint64_t i_ino, uint64_t index,
		  off_t offset)
{
	log_debug("fs_inode_truncate() called i_ino=%lu index=%lu offset=%ld",
		  i_ino, index, offset);
	int s;
	char key[128];
	snprintf(key, sizeof(key), "%lu.%lu", i_ino, index);
	if (offset == 0) {
		s = pmemkv_remove(ctx->db, key, strlen(key));
	...
	s = pmemkv_get(ctx->db, key, strlen(key), read_cb, &arg);
	if (s == PMEMKV_STATUS_NOT_FOUND) {
		s = pmemkv_put(ctx->db, key, strlen(key), arg.buf, arg.size);
		free(arg.buf);
		if (s == PMEMKV_STATUS_OUT_OF_MEMORY) {
			errno = ENOMEM;
			return (-1);
		...
		s = pmemkv_put(ctx->db, key, strlen(key), arg.buf, offset);
		free(arg.buf);
		if (s == PMEMKV_STATUS_OUT_OF_MEMORY) {
			errno = ENOMEM;
			return (-1);
